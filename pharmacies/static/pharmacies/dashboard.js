...
function renderPharmacyBills() {
    ...
    if (pharmacyBillsList.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" style="padding: 40px; text-align: center; color: var(--text-secondary);"><svg width="40" height="40" fill="none" stroke="#cbd5e1" stroke-width="1.5" viewBox="0 0 24 24" style="margin-bottom: 8px;"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="2"/></svg><br>No POS bills generated yet. Click <b>Create New Bill</b> to start.</td></tr>';
        return;
    }

    tbody.innerHTML = pharmacyBillsList.map(b => {
            ...
            <tr>
                ...
                <td style="padding: 12px 16px; text-align: center;">
                    <div style="display: inline-flex; gap: 6px; align-items: center; justify-content: center; width: 100%;">
                        <button class="btn-action" onclick="updateOrderStatusToDelivered('${escapeJSVal(o.order_id)}')" ${isDelivered ? 'disabled style="opacity: 0.5; cursor: not-allowed; background: var(--bg-tertiary); color: var(--text-muted);"' : 'style="background: #10b981; color: white;"'}>
                            Delivered
                        </button>
                        <button class="btn-action" style="background: var(--color-primary-glow); color: var(--color-primary);" onclick="viewOrderDetails('${escapeJSVal(o.order_id)}')">
                            View
                        </button>
                        <button class="btn-action" style="background: rgba(239, 68, 68, 0.1); color: #ef4444;" onclick="deletePharmacyOrder('${escapeJSVal(o.order_id)}')">
                            Delete
                        </button>
                    </div>
                </td>
