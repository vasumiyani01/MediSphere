...
import { Search, MapPin, Phone, Clock, Truck, Shield, ExternalLink, ShoppingBag } from 'lucide-react'

function PharmaciesSection({ isSlider, themeColor }) {
  const [search, setSearch] = useState('')
  const [deliveryOnly, setDeliveryOnly] = useState(false)

  const [dbPharmacies, setDbPharmacies] = useState([])
        ...
        if (data.success && data.pharmacies) {
          const formatted = data.pharmacies.map(p => {
            ...
            return {
              id: `db-${p.id}`,
              name: p.name,
              address: `${p.address || ''}, ${p.city || ''}, ${p.state || ''} - ${p.pincode || ''}`.replace(/^,\s*|,\s*$/, '').trim() || 'No address provided',
  ...
  const fallbackPharmacies = [
    { id: 1, name: 'Apollo Pharmacy', address: '12 Medical Square, Downtown', phone: '+1 (555) 019-2834', hours: 'Open 24/7', delivery: true, pickup: true, stockLevel: 'High', rating: 4.8 },
    { id: 2, name: 'Wellness Forever Medstore', address: 'Plot 42, Metro Galleria, West End', phone: '+1 (555) 014-9988', hours: 'Open 24/7', delivery: true, pickup: true, stockLevel: 'High', rating: 4.7 },
    { id: 3, name: 'MedPlus Pharmacy', address: '88 Greenwood Street, Sector 5', phone: '+1 (555) 012-7362', hours: '8:00 AM - 11:00 PM', delivery: false, pickup: true, stockLevel: 'Medium', rating: 4.5 },
    { id: 4, name: 'Guardian Care Pharmacy', address: '104 Lakeside Drive, Northern Heights', phone: '+1 (555) 015-8811', hours: '9:00 AM - 9:00 PM', delivery: true, pickup: true, stockLevel: 'Medium', rating: 4.3 },
    { id: 5, name: 'LifeSpring Pharmacy', address: '302 Oak Avenue, Riverdale', phone: '+1 (555) 018-2233', hours: 'Open 24/7', delivery: true, pickup: true, stockLevel: 'Low', rating: 4.6 },
    { id: 6, name: 'Apex Clinical Pharmacy', address: 'H-6, Apollo Annex, Health District', phone: '+1 (555) 011-9475', hours: '7:30 AM - 10:00 PM', delivery: false, pickup: true, stockLevel: 'High', rating: 4.9 },
  ]

  const allPharmacies = [...dbPharmacies, ...fallbackPharmacies]

  const filteredPharmacies = isSlider ? allPharmacies : allPharmacies.filter(pharm => {
            ...
            <div key={pharm.id} className="card" style={{
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                  ...
                  {pharm.pickup && (
                    <span className="badge" style={{ backgroundColor: `${themeColor || 'var(--color-primary)'}15`, color: themeColor || 'var(--color-primary)', border: `1px solid ${themeColor || 'var(--color-primary)'}30` }}>
                      <ShoppingBag size={10} style={{ marginRight: '4px' }} />
                      Store Pickup
                    </span>
                  )}
                </div>
